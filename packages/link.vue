...
export default {
  props: {
    ...
    linkDesc: [Function, null], //这是vue2的props多种类型语法，表明linkDesc可能是Function或者null
  ...
  mounted () {
    ...
    this.draw()
    // this.graph.add('mousemove', this.rootMousemove) // 也可以改成监听canvas或者document的mousemove
    // hook:beforeDestroy是vue隐藏事件，使用这个就能在组件destroy时收到触发信号，无需额外配置
  ...
  computed: {
    ...
    inPath: {
      get () {
        // TODO:返回值取决于mouseOnLink指向，而mouseOnLink指向又由inPath的set决定，感觉语义上有点混乱
  ...
  methods: {
    ...
    changeStyle () {  // 改变canvas大小，不如改名叫resizeCanvas
    ...
    initLine () { // 真正的绘制步骤，不如改名叫drawCanvas
      this.ctx.clearRect(0, 0, this.$el.width, this.$el.height)
      if (this.linkStyle) { // 这个分支没有走过，不知道设计目的是什么
    ...
    drawLine (strokeStyle) {
      ...
      this.currentPointList.forEach((point, idx) => {
        if (idx === 0) {
          ...
          ctx.lineTo(...point)
          ctx.stroke()  // stroke放到循环结束也没问题
        }
      })

      ctx.save()  // 为了后续画箭头方便，先保存目前的ctx状态，画完箭头再还原ctx。ctx只在mount的时候创建一次，所以需要不停维护。
    },

    drawDesc (color) {  //TODO:暂时没绘制文字的需求，不看
      const ctx = this.ctx
      let desc
    ...
    isPointInStroke (evt) {
      ...
      return this.link.isPointInLink(
        ...
        this.currentPathPointList //该参数可不传，默认使用link中已保存的PointList
      )
    },

    rootMousemove ( evt ) {
      this.inPath = this.isPointInStroke(evt)
      return this.inPath  // 这个返回值目前没用
    },

    linkMouseLeave(evt) { // 失去焦点也会触发该事件：右键菜单时，视为鼠标离开，否则鼠标移出canvas区域再取消菜单，就没法再取消mouseOnLink了，除非使用this.graph.add('mousemove',....）监视全局鼠标移动
      this.inPath=false;
    }
  },
  watch: {
    // 通过监视link.pathPointList实现移动node时link随之变化。不过有意思的是，link.pathPointList能够变化，也是因为
    // watch在不停获取它的值，而link.pathPointList的get方法中就会不停更新自己的值。也即，watch了link.pathPointList，
    // link.pathPointList才会变化，而link.pathPointList会变化，反过来导致watch重绘线条。
    'link.pathPointList' () {
      if(this.link.pathPointList == this.currentPointList)
        return
      this.draw()
    },
    inPath () {
